...
if ($matching_data_result && mysqli_num_rows($matching_data_result) > 0) {
    ...
    if(mysqli_num_rows($availability_result) > 0) {
        // booked
        echo "<script>
            alert('Error: Appointment set to be overlapping with other appointments . . . ');
            var app_id = '$app_id'; 
            window.location.href = '../A_EditAppointment.php?editID_app=' + app_id;
            </script>";
        exit;
    }else{
        ...
        if ($update_result) {
            ...
            echo "<script>
                ...
                window.location.href = '../A_AppointmentList.php';
                </script>";
            exit;
